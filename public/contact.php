...
$idRaw = isset($_GET['id']) && is_string($_GET['id']) ? trim($_GET['id']) : '';
$id = strtolower($idRaw);
if (!preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/', $id)) {
...
$queryUrl = $baseUrl . $scriptPath . '?id=' . rawurlencode($id);
...
$note = vcard_escape('Generated by Codecobble Team');
$uidValue = 'vcard-' . $id;
